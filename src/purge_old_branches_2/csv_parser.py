...
class CsvParser:
    def __init__(self, file: Path, ticket_col: str, status_col: str, done_status: str):
        self.file = file
    ...
    def done_tickets(self) -> list[str]:
        with open(self.file, "r", encoding="utf-8") as f:
            ...
            if self.ticket_col not in reader.fieldnames:
                raise ValueError(
                    f"Column '{self.ticket_col}' not found in CSV. "
            ...
            return [row[self.ticket_col] for row in reader if row[self.status_col] == self.done_status]
        return []
